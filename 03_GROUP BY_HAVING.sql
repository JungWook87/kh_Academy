...
-- 1) 부서코드만 조회
SELECT DEPT_CODE FROM EMPLOYEE; -- 23row

-- 2) 전체 급여 합 조회
SELECT SUM(SALARY) FROM EMPLOYEE; -- 1row
...
-- 직급코드 - 행이 여러개, 급여평균-AVG, 그룹함수, 1개, 인원수-COUNT, 그룹함수, 1개
SELECT JOB_CODE , ROUND(AVG(SALARY)), COUNT(*)  
FROM EMPLOYEE
GROUP BY JOB_CODE 
ORDER BY JOB_CODE;

SELECT JOB_CODE FROM EMPLOYEE; -- 23행
...
FROM EMPLOYEE
GROUP BY SUBSTR(EMP_NO, 8, 1) -- 별칭 사용 X (SELECT절 해석 전)
ORDER BY "인원 수"; -- 별칭 사용 O (SELECT절 해석 후)


---------------------------------------------------------------------------------------------------------

-- EMPOYEE 테이블에서 부서코드가 D5, D6인 부서의 평균 급여, 인원 수 조회
...
FROM EMPLOYEE
WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2000 -- 형변환이 없기 때문에 연산이 가장 빠르다
-- HIRE_DATE >= TO_DATE('2000-01-01') -- HIRE_DATE >= '2000-01-01' 자동형변환 -- 형변환 1번
...
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE로 묶은 그룹 중 급여 평균이 300만 이상인 그룹만 걸러냄
ORDER BY DEPT_CODE;
