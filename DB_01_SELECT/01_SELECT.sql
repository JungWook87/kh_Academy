...
-- SELECT : 조회할 때 쓰임
-- * : 전체, ALL( Ex. ID, NAME, NO, EMAIL... 모두).
-- *자리에 이름만 가져오고 싶으면 EMP_NAME을 넣으면 됨
-- FROM : 00테이블로 부터
...
-- 산술 연산은 숫자(NUMBER 타입)만 가능하다!, Java에서는 문자+숫자는 문자화
-- Java에서는 =는 대입 연산자, == 비교연산자였지만, DB에서는 =는 같다의 의미이다.
...
-- Ex) 게시판 작성시 등록시간 표시. 프론트에서 아이디와 내용을 DB로 보내주고, DB에서 아이디 조회 후
-- 아이디와 내용을 저장하면서 SYSDATE를 따져 기록한다.
...
--> 쌍따옴표 안에 작성되는 것들이 하나의 단어처럼 인식된다
...
-- D9은 여러개가 나올 수 있지만 D9-J1은 하나만 나온다

/*
SELECT DISTINCT DEPT_CODE , DISTINCT JOB_CODE FROM EMPLOYEE;
SELECT DEPT_CODE , DISTINCT JOB_CODE FROM EMPLOYEE;
-- ORA-00936: 누락된 표현식
*/
...
-- 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC(오름차순) | DESC(내림차순)] [NULLS FIRST | LAST]
...
-- EMPLOYEE 테이블에서 성이 '전'시인 사원의 사번, 이름 조회

SELECT EMP_ID , EMP_NAME 
FROM EMPLOYEE
WHERE EMP_NAME LIKE '전%';
-- WHERE EMP_NAME LIKE '전__'; 로 쓸수도 있지만 이름이 외자 혹은 4자인 사람은 검색이 X
...
-- ESCAPE
-- ESCAPE문자 뒤에 작성되 ㄴ_ 는 일반 문자로 탈출 한다는 뜻
-- ESCAPE문자 #, ^ 를 많이 씀
SELECT EMP_ID , EMP_NAME, EMAIL 
FROM EMPLOYEE
...
SELECT EMP_NAME 이름 , HIRE_DATE 입사일 -- AS 생략
-- SELECT EMP_NAME , HIRE_DATE -- 에러발생. 별칭이 안정해졌기 때문, 
FROM EMPLOYEE
ORDER BY 입사일;
