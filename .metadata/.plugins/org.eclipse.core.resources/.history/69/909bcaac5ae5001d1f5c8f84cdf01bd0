...
public class MemberServiceImpl implements  MemberService{
	...
	// 왜?? POJO의 위배는 외부 라이브러리를 상속받을 때 발생하고,
	// 내가 만든 것을 상속받을 때는 위배되지 않는다
	...
	public Member login(Member inputMember) {
		...
		// 이를 비교하는 기능(메서드)를 가지고 있어서 이를 활용하면 된다
		// 메서드이기 때문에 DB에서 사용하지 못하고 Service 단에서 비교하게 된다
		
		// ** Bcrypt 암호화를 사용하기 위해 이를 지원하는 Spring-security 모듈 추가 **
		// 메이븐 spring security core / web / config에 가서 5.7.1 버전 라이브러리 추가
		...
		if(loginMember != null) { // 일치하는 이메일을 가진 회원 정보가 있을 경우
			...
			if(bcrypt.matches(inputMember.getMemberPw(), loginMember.getMemberPw())) { // 비밀번호가 일치할 경우
				...
				loginMember.setMemberPw(null); // 비교 끝났으면 비밀번호 지우기
											   // 왜?? 로그인되면 멤버정보가 session에 올라가는데 비밀번호가
											   // 올라가면 안되므로
