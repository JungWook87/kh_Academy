...
function infoValidate(){
    ...
    // 닉네임 유효성 검사
    if(updateNickname.value.length == 0){ // 미작성 시 : 닉네임을 입력해주세요.
        alert("닉네임을 입력해주세요.");
        updateNickname.focus();
        return false;
    }

    if(!regExp1.test(updateNickname.value)){ // 유효하지 않은 경우
    ...
    // 전화번호 유효성 검사
    if(updateTel.value.length == 0){ // 미작성 시
        alert("전화번호를 입력해주세요.(- 제외)");
        updateTel.focus();
        return false;
    }

    if(!regExp2.test(updateTel.value)){ // 유효하지 않은 경우
        // alert(" 전화번호 형식이 올바르지 않습니다.");
        // updateTel.focus();
        // return false;

        return printAlert(updateTel, "전화번호 형식이 올바르지 않습니다.");
    }
