...
-- 제약조건을 특별히 추가하지 않았었도 시스템에서 알아서 제약조건명이 지정되어 있다

--> 수정은 별도 존재하지 않기 때문에 삭제 후 추가를 이용하여 수정
...
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE NULL; -- DEPT_TITLE 컬럼의 NOT NULL 제약조건 해제
...
INSERT INTO DEPT_COPY VALUES('D11', '개발2팀', 'L2', DEFAULT, DEFAULT);

SELECT * FROM DEPT_COPY;

DELETE FROM DEPT_COPY WHERE DEPT_ID = 'D11';
...
ROLLBACK;
SELECT * FROM TB2;
-- DML인 INSERT를 시행하고 COMMIT을 하지 않았고 ROLLBACK을 했지만,
-- 중간에 DDL인 컬럼명 변경을 시행하였기 때문에 COMMIT이 진행되었다
-- 그래서 ROLLBACK을 했음에도 추가된 행이 유지된다
