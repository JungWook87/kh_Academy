...
-- 선택 안된 컬럼은 값이 NULL이 들어간다(DEFAULT 존재 시 DEFAULT 값으로 삽입됨)
...
WHERE EMP_NAME IN ('유재식'); -- 3400000

SELECT BONUS FROM EMPLOYEE2
WHERE EMP_NAME IN ('유재식'); -- 0.2

SELECT SALARY FROM EMPLOYEE2
WHERE EMP_NAME IN ('방명수'); -- 1380000

SELECT BONUS FROM EMPLOYEE2
WHERE EMP_NAME IN ('방명수'); -- NULL

-- 방명수 급여, 보너스 수정
UPDATE EMPLOYEE2 SET
SALARY = (SELECT SALARY FROM EMPLOYEE2 WHERE EMP_NAME IN ('유재식')),
BONUS = (SELECT BONUS FROM EMPLOYEE2 WHERE EMP_NAME IN ('유재식'))
WHERE EMP_NAME = '방명수';
