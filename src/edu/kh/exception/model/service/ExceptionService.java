...
public class ExceptionService {
	...
	public void ex3() {
		...
		try {
			...
			// 예외처리 + 다형성
			// Exception 클래스 : 모든 예외의 최상위 부모로써
			// 다형성-업캐스팅 : 부모타입 참조변수로 자식객체를 참조
			
			// ** 상위 타입의 예외 클래스를 catch문에 작성하면 
			//    다형성 업캐스팅에 의해서 모두 잡아서 처리 가능
	...
	public void ex4() {
		// try ~ catch ~ finally
		// finally : try 구문에서 예외가 발생 하든말든 무조건 마지막에 실행 
		
		try {
			System.out.println(4 / 0); // ArithmeticExceotion 발생
		} catch(Exception e) {
	...
	public void methodC() throws IOException {
		...
		//Unhandled exception type IOException 예외가 처리가 안됐다
	...
	public void methodD() {
		...
		// throws IOException가 없는 methodC()와 비교
		
		// CheckedExceptino : 
		// RuntimeException과 자식 예외를 제외한 나머지 예외
		// - if문과 같은 단순 코드로 해결이 불가능하여
		//   반드시 예외처리 구문(try-catch)을 작성해야하는 예외
