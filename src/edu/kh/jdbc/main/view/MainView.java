...
public class MainView {
		...
		do {
			try {
				...
				if(LoginMember == null) { // 로그인 X
					...
					switch(input) {
					case 1 : 
						LoginMember = memberView.memberMenu(LoginMember); 
					...
					case 99 : System.out.println("프로그램 종료"); 
							// System.exit(0); // JVM 종료, 매개변수는 0, 아니면 오류를 의미. 미작성시 컴파일러가 자동 삽입
							System.exit(0);
							break;
					default : System.out.println("메뉴에 작성된 번호만 입력해주세요.");	
					}
	 ...
	 * 
	 * 1. 내 정보 조회 // select, where id = myid
	 * 2. 회원 목록 조회(아이디, 이름, 성별) // select, where id <> myid
	 * 3. 내 정보 수정(이름, 성별) // update
	 * 4. 비밀번호 변경(현재 비밀번호, 새 비밀번호, 새 비밀번호 확인) // update
	 * 5. 회원 탈퇴 //delete
	 * 
	 * ------------------------------------------------------------------
	 * 
