...
public class TCPServer {
	...
	// 		 순차적으로 데이터를 전달하고, 확인하고, 오류 발생 시 재전송
	...
	public void serverStart() {
		...
		try {
			//2. 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port);	// 서버용 소켓을 생성하여 포트와 결합
			...
			String clientIP = clientSocket.getInetAddress().getHostAddress();	// IP 주소형태 만들기
			...
			// InputStreamReader은 byte기반의 stream과 문자기반의 Stream을 연결에 사용하는 스트림
			...
			// close()도 밀어내는게 가능한데, 굳이 flush()를 사용하는 이유
			// -> 스트림을 닫지 않은 상태에서 내용을 내보내고 싶은 경우가 있음
			
			// 7-2) 클라이언트 -> 서버에게 입력한 메시지 전송 받기
			...
			//8. 통신 종료
			// 사용한 스트림, 소켓 자원을 모두 반한(close)
			try {
				// 보조스트림 close()시 연결된 기반스트림(is,os)도 같이 close 됨
				if(pw != null) pw.close();
				if(br != null) br.close();
