...
-- 1) 전 직원의 평균 급여 조회하기
SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE e ;				  
...
SELECT EMP_NAME , J.JOB_NAME , D.DEPT_TITLE  , SALARY 
FROM EMPLOYEE e 
...
-- 2) 사번, 이름, 부서, 직급, 급여 조회
SELECT EMP_ID , EMP_NAME , D.DEPT_TITLE , J.JOB_NAME , E.SALARY 
FROM EMPLOYEE e 
NATURAL JOIN JOB J
LEFT JOIN DEPARTMENT d ON (DEPT_CODE = D.DEPT_ID)
 ...
 *	SELECT D.DEPT_TITLE , SUM(E.SALARY)
	...
	LEFT JOIN DEPARTMENT d ON (E.DEPT_CODE = D.DEPT_ID)
	GROUP BY D.DEPT_TITLE
	HAVING SUM(E.SALARY) = 17700000, 80000 , 157800000; (X)
	--> HAVING SUM(E.SALARY) IN ( 17700000, 80000 , 157800000 );
...
SELECT E.EMP_ID , E.EMP_NAME , D.DEPT_TITLE , J.JOB_NAME , '사수' 구분
FROM EMPLOYEE e
LEFT JOIN DEPARTMENT d ON (E.DEPT_CODE = D.DEPT_ID)
LEFT JOIN JOB j USING(JOB_CODE)
...
SELECT E.EMP_ID , E.EMP_NAME , D.DEPT_TITLE , J.JOB_NAME , '사원' 구분
FROM EMPLOYEE e
LEFT JOIN DEPARTMENT d ON (E.DEPT_CODE = D.DEPT_ID)
LEFT JOIN JOB j USING(JOB_CODE)
...
-- 정렬을 하려고 할 때 RESULT SET에 대한 SELECT 구문이 필요함
...
FROM EMPLOYEE e 
WHERE E.ENT_YN = 'Y' -- 퇴직한 사람
AND SUBSTR(E.EMP_NO, 8, 1) = '2'; -- 여직원
									
-- 2) 퇴사한 여직원과 같은 부서, 같은 직급 (다중 열 서브쿼리)
SELECT E.EMP_NAME , E.JOB_CODE , E.DEPT_CODE , E.HIRE_DATE 
FROM EMPLOYEE e 
WHERE DEPT_CODE = (SELECT E.DEPT_CODE 
					FROM EMPLOYEE e 
					WHERE E.ENT_YN = 'Y' -- 퇴직한 사람
					AND SUBSTR(E.EMP_NO, 8, 1) = '2')
AND JOB_CODE = (SELECT E.JOB_CODE 
					FROM EMPLOYEE e 
					WHERE E.ENT_YN = 'Y' -- 퇴직한 사람
					AND SUBSTR(E.EMP_NO, 8, 1) = '2');
...
SELECT E.EMP_NAME , E.JOB_CODE , E.DEPT_CODE , E.HIRE_DATE 
FROM EMPLOYEE e 
WHERE DEPT_CODE = (SELECT E.DEPT_CODE , E.JOB_CODE
					FROM EMPLOYEE e 
					WHERE E.ENT_YN = 'Y' -- 퇴직한 사람
					AND SUBSTR(E.EMP_NO, 8, 1) = '2');
...
(SELECT EMP_NAME FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID)
FROM EMPLOYEE MAIN;
