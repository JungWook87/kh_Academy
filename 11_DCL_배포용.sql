...
CREATE USER kjo_sample IDENTIFIED BY sample1234;


-- 2. 새 연결(접속 방법) 추가
--> ORA-01045: 사용자 KJO_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다

-- 3. 접속 권한 부여
-- [권한 부여 작성법]
-- GRANT 권한, 권한, ... TO 사용자명;
GRANT CREATE SESSION TO kjo_sample;
...
GRANT CREATE TABLE TO kjo_sample;

ALTER USER kjo_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM; -- 할당
...
GRANT CONNECT, RESOURCE TO kjo_sample;
...
-- kh / kjo_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh) kjo_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO kjo_sample;


-- 3.(sample) 다시 kh>EMPLOYEE 테이블 조회
SELECT * FROM kh.EMPLOYEE;

SELECT * FROM kh.DEPARTMENT;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다 -> DEPARTMENT 테이블 조회 권한은 준적 없으므로 불가
...
REVOKE SELECT ON EMPLOYEE FROM kjo_sample;
